    ...
    private boolean hasApiSupport() {
        try {
            ...
            if (server.hasCodeInsightsApi()) {
                ...
                LOGGER.info(format("Bitbucket Server version is to old: %s . The minimum version that supports code insights is %s",
                        server.getVersion(), ServerProperties.CODE_INSIGHT_VERSION));
            }
        } catch (IOException e) {
